...
var ctrl = app.controller('myCtrl', function ($scope, $http, $localStorage) {
    ...
    $scope.getcustomerlist = function (flag) {
        ...
        $http.get('/api/Custome/GetCustomerList?curpage=' + curpage + '&maxrows=' + selecting).then(function (res, data) {
            $scope.Userlist = res.data.Table;
            $scope.paggin = res.data.Table1;
            if ($scope.Userlist.length < selecting) {
                $scope.secondvalue = $scope.secondvalue - (selecting - $scope.Userlist.length);
